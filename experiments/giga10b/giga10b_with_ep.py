...
class FastMoERouterAdapter(nn.Module):
    def __init__(self, giga_router, capacity_factor: float = 1.0):
        super().__init__()
...
class PipelineMoEBlock(nn.Module):
    def __init__(self, cfg: "Giga10BConfig", group: dist.ProcessGroup, streams: dict):
        ...
        # Identity ops for GigaChat architecture compatibility
        self.pre_ops = nn.Identity()
        self.post_ops = nn.Identity()
        self.moe_norm = nn.Identity()
    ...
    def _fwd_stage_pre_ops(self, mb_idx, ctx, chunks, ev_signal):
        if 0 <= mb_idx < self.cfg.moe.micro_batches:
            with torch.cuda.stream(self.streams[Streams.COMPUTE]):
                x_mb = chunks[mb_idx]
                with torch.enable_grad():
                    permuted_inputs, permuted_weights, gather_index, capacity = self.router(x_mb)
            ...
            ev_signal[mb_idx].record(self.streams[Streams.COMPUTE])

    def _fwd_stage_dispatch(self, mb_idx, ctx, ev_wait, ev_signal):
        if 0 <= mb_idx < self.cfg.moe.micro_batches:
            self.streams[Streams.COMM].wait_event(ev_wait[mb_idx])
            with torch.cuda.stream(self.streams[Streams.COMM]):
                permuted_local = ctx[mb_idx]["permuted_inputs"]
                capacity = ctx[mb_idx]["capacity"]
                ...
                dist.all_to_all_single(reshaped_out, reshaped_in, group=self.group, async_op=False)
                ctx[mb_idx]["dispatch_output"] = reshaped_out.view(-1, self.hidden_dim)
            ev_signal[mb_idx].record(self.streams[Streams.COMM])

    def _fwd_stage_experts(self, mb_idx, ctx, ev_wait, ev_signal):
        if 0 <= mb_idx < self.cfg.moe.micro_batches:
            self.streams[Streams.COMPUTE].wait_event(ev_wait[mb_idx])
            with torch.cuda.stream(self.streams[Streams.COMPUTE]):
                disp_out = ctx[mb_idx]["dispatch_output"]
                capacity = ctx[mb_idx]["capacity"]
                ...
                ctx[mb_idx]["input_experts"] = expert_input_grouped.detach()
                with torch.enable_grad():
                    res = [
                        self.experts[i](expert_input_grouped[i])
                ...
                ctx[mb_idx]["expert_output"] = expert_out_4d.reshape(-1, self.hidden_dim).detach()
            ev_signal[mb_idx].record(self.streams[Streams.COMPUTE])
    ...
    def _fwd_stage_combine(self, mb_idx, ctx, ev_wait, ev_signal):
        if 0 <= mb_idx < self.cfg.moe.micro_batches:
            self.streams[Streams.COMM].wait_event(ev_wait[mb_idx])
            with torch.cuda.stream(self.streams[Streams.COMM]):
                expert_out = ctx[mb_idx]["expert_output"]
                capacity = ctx[mb_idx]["capacity"]
                tokens_per_rank = self.num_local_experts * capacity
                reshaped_in = expert_out.view(self.cfg.world_size, tokens_per_rank, self.hidden_dim)
                reshaped_out = torch.empty_like(reshaped_in)
                dist.all_to_all_single(reshaped_out, reshaped_in, group=self.group, async_op=False)
                ctx[mb_idx]["combined_output"] = reshaped_out.view(-1, self.hidden_dim)
            ev_signal[mb_idx].record(self.streams[Streams.COMM])
    ...
    def _fwd_stage_post_ops(self, mb_idx, ctx, outputs, ev_wait, chunks):
        if 0 <= mb_idx < self.cfg.moe.micro_batches:
            self.streams[Streams.COMPUTE].wait_event(ev_wait[mb_idx])
            with torch.cuda.stream(self.streams[Streams.COMPUTE]):
                moe_out = ctx[mb_idx]["combined_output"]
                gather_index = ctx[mb_idx]["gather_index"]
                weights = ctx[mb_idx]["permuted_weights"]
                weighted_moe = moe_out * weights.unsqueeze(1)

                output_buffer = torch.zeros_like(chunks[mb_idx])
                valid_mask = gather_index != -1
                output_buffer.index_add_(0, gather_index[valid_mask], weighted_moe[valid_mask])

                outputs[mb_idx] = output_buffer
                ctx[mb_idx]["combined_output_for_grad"] = moe_out

    # --- BWD Stages ---
    def _bwd_stage_post_ops(self, mb_idx, ctx, grad_chunks, ev_signal):
        if 0 <= mb_idx < self.cfg.moe.micro_batches:
            with torch.cuda.stream(self.streams[Streams.COMPUTE]):
                ctx[mb_idx]["grad_moe_scattered"] = grad_chunks[mb_idx]
            ev_signal[mb_idx].record(self.streams[Streams.COMPUTE])
    ...
    def _bwd_stage_combine(self, mb_idx, ctx, ev_wait, ev_signal):
        if 0 <= mb_idx < self.cfg.moe.micro_batches:
            self.streams[Streams.COMM].wait_event(ev_wait[mb_idx])
            with torch.cuda.stream(self.streams[Streams.COMM]):
                d_moe_scattered = ctx[mb_idx]["grad_moe_scattered"]
                gather_index = ctx[mb_idx]["gather_index"]
                weights = ctx[mb_idx]["permuted_weights"]
                capacity = ctx[mb_idx]["capacity"]
                moe_out = ctx[mb_idx]["combined_output_for_grad"]
                ...
                valid_mask = gather_index != -1
                d_weighted_moe[valid_mask] = d_moe_scattered[gather_index[valid_mask]]
                d_moe_out = d_weighted_moe * weights.unsqueeze(1)

                ctx[mb_idx]["grad_permuted_weights"] = (d_weighted_moe * moe_out).sum(dim=1)

                tokens_per_rank = self.num_local_experts * capacity
                reshaped_in = d_moe_out.view(self.cfg.world_size, tokens_per_rank, self.hidden_dim)
                reshaped_out = torch.empty_like(reshaped_in)
                dist.all_to_all_single(reshaped_out, reshaped_in, group=self.group, async_op=False)
                ctx[mb_idx]["grad_expert_out"] = reshaped_out.view(-1, self.hidden_dim)
            ev_signal[mb_idx].record(self.streams[Streams.COMM])
    ...
    def _bwd_stage_experts(self, mb_idx, ctx, ev_wait, ev_signal):
        if 0 <= mb_idx < self.cfg.moe.micro_batches:
            self.streams[Streams.COMPUTE].wait_event(ev_wait[mb_idx])
            with torch.cuda.stream(self.streams[Streams.COMPUTE]):
                d_expert_out_flat = ctx[mb_idx]["grad_expert_out"]
                capacity = ctx[mb_idx]["capacity"]
                d_expert_out_grouped = (
                    d_expert_out_flat.view(
                        self.cfg.world_size, self.num_local_experts, capacity, self.hidden_dim
                    )
                    .transpose(0, 1)
                    .reshape(self.num_local_experts, -1, self.hidden_dim)
                )
                inp = ctx[mb_idx]["input_experts"].detach().requires_grad_(True)
                with torch.enable_grad():
                    res = [self.experts[i](inp[i]) for i in range(self.num_local_experts)]
                    out = torch.stack(res, dim=0)
                grads = torch.autograd.grad(out, inp, d_expert_out_grouped)
                d_dispatch_out = (
                    grads[0]
                    .view(self.num_local_experts, self.cfg.world_size, capacity, self.hidden_dim)
                    .transpose(0, 1)
                    .reshape(-1, self.hidden_dim)
                )
                ctx[mb_idx]["grad_dispatch_out"] = d_dispatch_out
            ev_signal[mb_idx].record(self.streams[Streams.COMPUTE])
    ...
    def _bwd_stage_dispatch(self, mb_idx, ctx, ev_wait, ev_signal):
        if 0 <= mb_idx < self.cfg.moe.micro_batches:
            self.streams[Streams.COMM].wait_event(ev_wait[mb_idx])
            with torch.cuda.stream(self.streams[Streams.COMM]):
                d_disp = ctx[mb_idx]["grad_dispatch_out"]
                capacity = ctx[mb_idx]["capacity"]
                tokens_per_rank = self.num_local_experts * capacity
                reshaped_in = d_disp.view(self.cfg.world_size, tokens_per_rank, self.hidden_dim)
                reshaped_out = torch.empty_like(reshaped_in)
                dist.all_to_all_single(reshaped_out, reshaped_in, group=self.group, async_op=False)
                ctx[mb_idx]["grad_permuted_input"] = reshaped_out.view(-1, self.hidden_dim)
            ev_signal[mb_idx].record(self.streams[Streams.COMM])
    ...
    def _bwd_stage_pre_ops(self, mb_idx, ctx, ev_wait, dx_list):
        if 0 <= mb_idx < self.cfg.moe.micro_batches:
            self.streams[Streams.COMPUTE].wait_event(ev_wait[mb_idx])
            with torch.cuda.stream(self.streams[Streams.COMPUTE]):
                d_permuted = ctx[mb_idx]["grad_permuted_input"]
                x_in = ctx[mb_idx]["input_pre"]
                gather_index = ctx[mb_idx]["gather_index"]
                d_permuted_weights = ctx[mb_idx]["grad_permuted_weights"]

                with torch.enable_grad():
                    _, permuted_weights_graph, _, _ = self.router(x_in)
